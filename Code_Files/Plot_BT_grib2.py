...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 19 17:23:08 2018
...
def download_HRRR_variable_from_pando(DATE, variable,
    ...
    for h in hours:
        for f in fxx:
            ...
            try:
                try:
                    idxpage = urllib.request.urlopen(idxfile)
                except:
                    ...
                    idxpage = urllib.request.urlopen(idxfile, context=ctx)
            ...
            if not any(variable in s.decode("utf-8") for s in lines):
                print("\n   ERROR!!!")
            ...
            for g in lines:
                ...
                #if expr.search(g):
                if expr.search(g.decode("utf-8")):
                    parts = g.decode("utf-8").split(':')
                    rangestart = parts[1]
                    parts = lines[gcnt+1+more_vars].decode("utf-8").split(':')
...
download_HRRR_variable_from_pando(DATE, variable,
                                  hours=hours, fxx=fxx,
                                  model='hrrr', field=field,
                                  more_vars=0,
                                  outdir='./')

#PLOT
path = '%sfromPando_%s_h%02d_f%02d_%s.grib2' \
                       % (model.upper(), DATE.strftime('%Y%m%d'), hours[0], fxx[0], variable.replace(':', '_').replace(' ', '_'))

grbs = pygrib.open(path)
grb = grbs.readline()

#Get brightness temperature values and latitudes and longitudes
Tb = grb.values 
...
x, y = m(lons,lats)
cs = m.pcolormesh(x,y,Tb,shading='flat',cmap=plt.cm.jet)
m.drawcoastlines()
m.drawcountries()
m.drawstates()
m.drawmapboundary()
